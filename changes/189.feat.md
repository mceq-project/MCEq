...
pre-fetches packages before submitting cluster jobs. `config.mceq_db_fname` now defaults to
`mceq_base_air_1d_v2.h5`; a monolithic database still works and is resolved
against itself. `pyyaml` becomes a dependency. The test suite and the example
notebooks run on a CI-sized cut of the release data (`mceq_ci_*_v2.h5`),
cached in CI under a manifest-digest key.

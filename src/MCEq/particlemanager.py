...
The species layer lives in ``species/constants.py`` (``_pdata``,
``ATOMIC_MASS_UNIT_G``, ``_pname``), ``species/particle.py`` and
``species/manager.py``. This module re-exports the pinned surface (D14,
Phase 7 deletes it). ``_pdata`` re-binds the *same* table object the species modules
...
#: public surface the recorded contract lists (privates are re-exported above

...
def test_solve_batch_reuses_density_instances(mceq_sib21):
    """Density-model *instances* may be reused across solve_batch calls.

    Regression test: the zenith angle is cached on the density model, so
    switching between two instances that both already carry the batch's
    zenith left set_zenith_azimuth with nothing to invalidate, and every
    condition silently inherited the path built for the first one.
    """
    saved_kernel = config.kernel_config
    dm_before = mceq_sib21.density_model
    try:
        config.kernel_config = "numpy_etd2"
        models = [
            dprof.CorsikaAtmosphere("BK_USStd", None),
            dprof.CorsikaAtmosphere("PL_SouthPole", "January"),
        ]
        # The zenith must match the instance's own ``theta_deg`` for the
        # regression: set_density_model re-applies that angle, which used to
        # be the only thing clearing the path.
        zenith = float(mceq_sib21.theta_deg)
        conditions = [{"zenith_deg": zenith, "density_model": dm} for dm in models]

        first = mceq_sib21.solve_batch(conditions=conditions)
        # Same instances again, now carrying theta_deg == 60 from above.
        second = mceq_sib21.solve_batch(conditions=conditions)

        np.testing.assert_array_equal(first.nsteps_per_col, second.nsteps_per_col)
        np.testing.assert_allclose(first.sol, second.sol, rtol=1e-12, atol=0)
        assert not np.allclose(second.sol[:, 0], second.sol[:, 1]), (
            "both conditions solved the same atmosphere"
        )
    finally:
        config.kernel_config = saved_kernel
        mceq_sib21.set_density_model(dm_before)

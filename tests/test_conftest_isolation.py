...
autouse restore ran *after* the session fixture had mutated ``config`` and so
preserved the mutation for every later test in the same xdist worker
(``disabled_particles = []``, ``mkl_threads = 2``, the reduced database, ...).

...
def convert_to_compact(fname):
    ...
    if fname.endswith('_ledpm.bz2'):
        ...
        if dbg > 0: print "convert_to_compact(): Low energy extension requested", fname
        
        dpmpath = os.path.join(config['data_dir'], 
            config['low_energy_extension']['le_model'].translate(None, "-.").upper() 
            + '_yields_compact.bz2')
        ...
        try:
            ...
            raise Exception("convert_to_compact(): Error, low-energy model file expected but" +
                "not found.\n", dpmpath)
    ...
    # expected format
    fn_he = fname.replace('.ppd','.bz2').replace('_compact','').replace('_ledpm','')
        ...
        for d in dec_di[interm_mothers[-1]]:
            ...
            if d not in standard_particles:
                ...
                if abs(d) in [12, 13, 14, 16]:
                    ...
                    is_prompt = sum([part_d.ctau(mo) <= ctau_pr for mo in interm_mothers]) > 0
                    if is_prompt:
...
def extend_to_low_energies(he_di=None, le_di=None, fname=None):
    ...
    if fname:
        ...
        he_di = pickle.load(BZ2File(fname.replace('_ledpm','').replace('.ppd','.bz2')))
        
        # Load low energy model yields
        le_di = pickle.load(BZ2File(os.path.join(config['data_dir'], 
            config['low_energy_extension']['le_model'].translate(None, "-.").upper() 
            + '_yields.bz2')))
    ...
    intp_indices = np.arange(transition_idx - nbins_interp/2 - 1,
                             transition_idx + nbins_interp/2 + 1.1, 1, 
                             dtype='int32')
